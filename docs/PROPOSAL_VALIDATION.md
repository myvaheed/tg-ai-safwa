...
flowchart TD
    ...
    D --> E{"Есть зависимое решение<br/>или незавершённая работа?"}
...
отклонил неточное время, это коррекция предложения, а не отказ от напоминания вообще.
...
и факт незавершённой проверки. Следующий запрос использует эти результаты, но не возобновляет
старые Proposals самопроизвольно.
...
| Пользователь уточнил отклонённое предложение | Учитывается коррекция, а не выдуманный отказ от намерения |
| Исправление не даёт прогресса | Ограниченная остановка с перечислением нерешённого |
| Проверка прервана после Save | Сохранённое остаётся; непроверенная часть не объявляется проверенной |
