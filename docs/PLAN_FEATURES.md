...
| **The retrospective — the statistics half, shipped** | M | Effort taken, finished and its share; initial, added and taken out; Actions finished, remaining and blocked; Passed and Missed per Check series tied to a Value, over the Sprint's days. Read off the record, no model involved. The AI analysis half shipped in Wave 5. |
...
| **Onboarding — the first start and a return after an absence, shipped** | L | The first start shipped 2026-09-24 ([OB-NOTICE-001](../tests/brd/onboarding.feature)…[OB-MANUAL-006](../tests/brd/onboarding.feature)): a notice before the first answer, a tip after each created or finished item, a subagent shown as is. The return shipped 2026-09-25 ([OB-RETURN-007](../tests/brd/onboarding.feature)): the owner's first message after 14 days away is followed by what stands in Today and the Sprint, as a block a hook's request carries ([AG-HOOK-048](../tests/brd/tg_agent_shell/agents.feature)), with its own switch. One table came, `owner_presence`, which startup adds; no rebuild. |
...
| **8 — Similar existing items on the screen that creates one, shipped** | L | Shipped 2026-09-25 as [PR-SIMILAR-030](../tests/brd/tg_agent_shell/proposals.feature): the review screen lists the open items of the same type most like the new one, by a local model loaded in the background, with no interception and nothing the model reads. One dependency came, `fastembed`, and no column. |
