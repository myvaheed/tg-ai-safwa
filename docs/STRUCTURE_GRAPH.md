...
`Value` :79 · `Card` :89 · `CardValue` :133 · `Tag` :143 · `CardTag` :152 · `SavedRequest` :160 ·
...
Views: `ai_cards`, `ai_tags`, `ai_values`, `ai_requests`, `ai_current_sprint`,
`ai_current_sprint_metrics`, `ai_card_events`.

### `ai/service.py` — agent loop + proposals

Tool schemas: `QUERY_SAFWA_TOOL` :93 · `MUTATION_TOOL_DESCRIPTIONS` :115 · `MUTATION_TOOLS` :127 ·
`SAFWA_TOOLS` :138. (`MAX_TOOL_CALLS`, `MAX_REPAIR_ROUNDS`, `SUSPENDED_BATCH_LOOKUP_LIMIT` are in
`constants.py`.)
...
### `telegram/items.py` — shared Tag/Value screens

`render_item_editor` :21 · `render_item_text_prompt` :155.
...
no field controls) · `render_ai_outcome` :309 · **`continue_agent_approval`** :326 (resumes the model
after the last queue item).

### `telegram/commands.py` — `@router.message` handlers

`end_subsession` :63 · `/start` :85 · `/newsession` :96 · `/endsession` :126 · `/today` :174 ·
`/backlog` :179 · `/sprint` :184 · `command_add` :240 (nav only) · `/advisor` :245 · `/values` :257 ·
`/tags` :294 · `/requests` :325 · `render_saved_request` :357 · `/memory` :404 · `/syncmem` :417 ·
