...
Начало реализации, 2026-09-15. Этапы 1–2 и батчи 1–3 реализованы: общий реестр, автоматический
Summary, предложение Heavy analyzer, выключатели в Профиле, первая инициатива — блокер — и
первая проверка по времени — Цели без Действий. Хук, чей эффект доходит до агента —
OfferTool или Advise, — включается и выключается целиком, в Профиле, руками владельца; хук,
делающий свою работу (Run), всегда включён. Инициатива — одна просьба к Advisor,
...
- [hooks/registry.py](../src/tg_agent_shell/hooks/registry.py) проверяет подключения и
  ...
  OfferTool или Advise), перед evaluate и перед словами спрашивает политику приложения; Safwa
...
- Tick — наступление дневной проверки: подписка OnTick() времени не называет, его называет
  приложение — читатель TickTime, который реестр получает при сборке вместе с политикой
  выключателей. Safwa отдаёт из [profile/api.py](../src/safwa/features/profile/api.py) поле
  «Morning time» Профиля (MORNING_TIME_DEFAULT = "09:00"). Один цикл оболочки — run_ticks в
  [cues/initiatives.py](../src/tg_agent_shell/cues/initiatives.py) над TickSchedule из
  [hooks/ticks.py](../src/tg_agent_shell/hooks/ticks.py) — на каждом обороте читает это время
  и отдаёт Tick тем же queue_advice, что и Committed. Последний взгляд хранится в памяти
  процесса, запуск считается первым: время, прошедшее при остановленном или выключенном хуке,
  не догоняется, а перенесённое владельцем время действует со следующего наступления без
  перезапуска. Потребители Advise по времени — Цели и Подцели без Действий и Hard Time вне
  плана в том же cards/hooks.py: evaluate возвращает одну отметку «пора», а список читает
  prepare в момент доставки.

Это четыре поддержанные пары событие/реакция. Неподдержанное подключение отвергается при сборке,
в том числе после route: этот вызов пока исполняется внутри agent_runtime и не проходит через
адаптер событий. До появления реального потребителя нет заглушки RefuseTool. Прежние
низкоуровневые before_tool/after_tool остаются самостоятельными портами наблюдения; ими новый
реестр не регистрируется.
...
агента — OfferTool даёт сессии помощника, Advise подаёт Advisor просьбу, — владелец включает и
выключает целиком, руками в Профиле, как остальные настройки (свойство agent_related у
определения). Профиль показывает каждый такой хук по названию и описанию — предложение Heavy
analyzer среди них. Хук, делающий свою работу (Run), в Профиле не показан и всегда включён —
автоматический Summary такой: сворачивание окна диалога — нужда самой Safwa, не помощь
владельцу. В Профиле хранится набор выключенных хуков; новому хуку своя колонка не нужна.
...
Ниже описан целевой контракт всех этапов. Реализованы четыре пары из раздела «Что работает
сейчас»; подписки называются OnAfterTurn, OnAfterTool, OnCommitted и OnTick, чтобы данные
...
BeforeTool — проектное название; класса в коде пока нет. Подписка Tick несёт расписание
проверки: время объявляется в том же on, а не в отдельном списке таймеров, и «последний
взгляд» цикла живёт в памяти процесса, не в таблице и не в системном промпте.
...
не заводит собственный poll. Время одно на все дневные хуки, и назначает его владелец в
Профиле; расчёт использует часы и часовой пояс рабочего пространства. Цикл помнит последний
...
| Heavy analyzer | AfterTool → OfferTool | Не превращать предложение помощника в обязательный анализ; выключатель в Профиле, как у всякого хука с эффектом для агента |
...
цикла тиков через TickTime; подписка OnTick() времени больше не называет. Затем три хука,
каждый — определение, предметное чтение и строка подключения:
...
- Hard Time вне плана ([PL-HARDTIME-021](../tests/brd/planning.feature)): одно определение с
  двумя подписками — OnCommitted(kind=SPRINT_STARTED), факт пишет start_sprint, и OnTick().
...
[AG-HOOK-035](../tests/brd/tg_agent_shell/agents.feature)). Run на Committed и Tick реестр
откроет с первым потребителем — истечением спринта в следующем батче.
...
Регистрация, три действующие реакции, выключатели, ожидающая инициатива и дневная проверка
проверяются в [test_hooks.py](../tests/test_hooks.py),
[test_initiatives.py](../tests/test_initiatives.py), [test_summary.py](../tests/test_summary.py),
[test_profile_ui.py](../tests/test_profile_ui.py), [test_cards.py](../tests/test_cards.py),
[test_checks.py](../tests/test_checks.py), [test_sprint.py](../tests/test_sprint.py),
...
(EMPTY_PARENT_GRACE_DAYS) и окно Hard Time (HARD_TIME_NOTICE_DAYS) — константы своих фич, час
проверки — поле Профиля; базовый контракт их не знает.
