...
- Tick — наступление дневной проверки: OnTick(at="HH:MM") называет местное время рабочего
  пространства, один цикл оболочки — run_ticks в
  [cues/initiatives.py](../src/tg_agent_shell/cues/initiatives.py) над TickSchedule из
  [hooks/ticks.py](../src/tg_agent_shell/hooks/ticks.py) — отдаёт Tick тем же queue_advice,
  что и Committed. Последний взгляд хранится в памяти процесса, запуск считается первым:
  время, прошедшее при остановленном или выключенном хуке, не догоняется. Второй потребитель
  Advise — Цели и Подцели без Действий в том же cards/hooks.py: evaluate возвращает одну
  отметку «пора», а список читает prepare в момент доставки.
