...
Начало реализации, 2026-09-15. Этапы 1–2 и батчи 1–2 реализованы: общий реестр, автоматический
Summary, предложение Heavy analyzer, выключатели в Профиле и первая инициатива — блокер. Хук с
...
- Committed — факт из [foundation/changes.py](../src/tg_agent_shell/foundation/changes.py):
  ...
  держит одну ожидающую инициативу хука строкой Cue — имя хука и данные, без слов; перед
  доставкой очередь спрашивает у реестра слова (prepare фичи), проверив выключатель.
  Первый потребитель — блокер Карточек в [cards/hooks.py](../src/safwa/features/cards/hooks.py).

Это три поддержанные пары событие/реакция. Неподдержанное подключение отвергается при сборке,
в том числе после route: этот вызов пока исполняется внутри agent_runtime и не проходит через
адаптер событий. До появления реального потребителя нет заглушек Tick и RefuseTool. Прежние
...
Tick и BeforeTool — проектные названия; классов в коде пока нет. Подписка Tick
...
Регистрация, три действующие реакции, выключатели и ожидающая инициатива проверяются в
[test_hooks.py](../tests/test_hooks.py), [test_initiatives.py](../tests/test_initiatives.py),
[test_summary.py](../tests/test_summary.py), [test_profile_ui.py](../tests/test_profile_ui.py),
[test_cards.py](../tests/test_cards.py),
...
с запрещённым импортом Safwa. Строки о Tick — будущие.
