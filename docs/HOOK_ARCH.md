...
и предложение Heavy analyzer. События сохранения, расписания, инициативы и журнал ниже остаются
проектом следующих этапов. Основание — [FUTURE_FEATURE.md](FUTURE_FEATURE.md),
...
- [hooks/registry.py](../src/tg_agent_shell/hooks/registry.py) проверяет подключения, сохраняет
  полный каталог и индексирует включённые определения по типу события. Фильтры применяются до
...
- AfterTool содержит сессию-источник, роль и вид агента, вызов, копию результата и исход.
  ...
  OnAfterTool фильтрует инструмент, роль и исход. OfferTool сейчас разрешён только после
...
- `/status` показывает определения из реестра, включая выключенные. Run после ошибки сообщает,
  что уже доставленный ответ остаётся в силе; ошибка необязательного предложения помощника
  попадает в журнал приложения и сохраняет результат чтения. Отмена не поглощается реестром.
...
тех же хуков в FeatureModule нет. MODULES подключает возможности, HOOKS — автоматические реакции.
...
| evaluate | Асинхронная проверка: получить событие и доступ к чтению, вернуть подходящие данные или пустой результат |
| effect | Один из поддерживаемых способов исполнения этих данных |

В условной типизации evaluate принимает конкретный тип события E и возвращает последовательность
...
подписан на Committed и Tick, его evaluate явно разбирает эти два варианта. В первом он проверяет
...
HEAVY_ANALYZER_HOOK = HookSpec(
    ...
    on=(OnAfterTool(agent="root", tool="query_data", outcome="success"),),
...
Во втором evaluate проверяет форму/объём уже полученного результата, а координатор предлагает
существующего помощника.
В третьем evaluate находит конкретный период блокировки, а refresh перед разговором подтверждает,
...
flowchart TD
    ...
    B --> D
    D --> E["evaluate: факты или пустой результат"]
    E -->|Пусто| Z["Закончить"]
...
фильтрация проверяются до evaluate, журнал — до семантической реакции.
...
Это ограничение относится к перехвату обычного создания из кандидата 8.
...
- Discard зависимого Proposal не доказывает отказ от всего исходного намерения. Для цепочки
  сохраняются фактические исходы и текст уточнения пользователя.
...
краткому промежуточному нулю в таблице. От расписания Hard Time зависит соответствующая часть
порядка показа Today, а не сама связь с критерием.

## 10. Последовательность реализации

Каждый этап должен давать проверяемое поведение. Большой универсальный движок до первого
реального хука не нужен. Этапы 1–2 выполнены; этапы 3–6 описывают дальнейшую работу.

### Этап 1. Контракт и единственный список подключений — реализован

Оболочка содержит типы определения, подключения, подписки и двух работающих реакций, проверку
регистрации и индекс подписок. [Registry](../src/tg_agent_shell/registry.py) принимает список
из [bootstrap/modules.py](../src/safwa/bootstrap/modules.py). Диагностика читает его напрямую.
...
хук виден в каталоге, но evaluate не вызывается; приложение-пример без Safwa собирается с пустым
...
Оба прежних автоматических пути удалены: поле after_turn больше не регистрирует Summary,
а SQL-чтение не содержит специальной проверки предложения помощника. Низкоуровневые наблюдатели
инструментов сохранены; их прежние сценарии продолжают проверяться.
...
Это целевой набор проверок всех этапов. Регистрация и две действующие реакции проверяются
в [test_hooks.py](../tests/test_hooks.py), [test_summary.py](../tests/test_summary.py),
[test_heavy_analyzer_e2e.py](../tests/e2e/test_heavy_analyzer_e2e.py) и приложении-примере
с запрещённым импортом Safwa. Строки об инициативах, сохранениях и решениях остаются будущими.

| Ситуация | Ожидаемый результат |
|---|---|
| Выключенный хук и подходящее событие | evaluate не вызывается; выключение видно в каталоге |
...
Центральный список и две действующие реакции реализованы. Оставшиеся реакции добавляются
вместе со своими потребителями и проверяемыми правилами исполнения.
...
Критерий готовности общего решения: новый обычный хук требует собственного evaluate, необходимых
