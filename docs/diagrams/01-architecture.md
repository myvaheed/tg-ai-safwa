...
**`safwa/features/*`** — тринадцать пакетов в `MODULES`, каждый со своими правилами в
`tests/brd/`. Плюс `advisor`, у которого `MODULE` нет: он корневая сессия, а не то, что
подключается к реестру.
