...
| **`proposals/`** | подготовка, review, экран, разрешение |
| **`features/<x>/proposal.py`** | что этот вызов значит для этой сущности |

Advisor не держит инструментов изменения вообще. `workspace_mutator` предлагает всё, что относится к рабочему пространству —
