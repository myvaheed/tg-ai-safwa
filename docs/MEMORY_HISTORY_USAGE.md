...
- Check resolution is the one proposal screen that carries field controls. The model can propose which Pending Checks to answer, but not the answers, so each row is tappable and cycles Passed, Missed, and Not applicable before Save. This is a deliberate exception to the read-only rule, not an oversight.
