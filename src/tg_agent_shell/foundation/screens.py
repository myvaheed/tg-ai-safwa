"""An item that can be put on a screen and cited.

Declared by the feature that owns it, assembled by the composition root, and read by the
adapters and the `open` tool. It is here rather than in `telegram/` because the engine
names it too and reaches no further than `foundation/`. How a screen is reached from
Telegram is `telegram/contributions.py`.
"""
