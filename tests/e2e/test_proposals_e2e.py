...
async def test_invalid_create_returns_minimal_repair_arguments_to_the_model(e2e_harness):
    ...
    advisor, provider = e2e_harness.advisor([route_turn("workspace_mutator"), invalid, repaired])
...
async def test_child_proposal_fails_cleanly_when_earlier_parent_is_discarded(e2e_harness):
    ...
    advisor, provider = e2e_harness.advisor(
        [
            route_turn("workspace_mutator"),
            response,
            "The Goal was discarded, so I did not retry its child Action.",
            "The Goal was discarded, so I did not retry its child Action.",
        ]
    )
...
async def test_new_tag_and_dependent_card_link_use_one_repair_round(e2e_harness):
    ...
    advisor, provider = e2e_harness.advisor(
        [
            ...
            first_turn,
            repaired_turn,
            "The Tag was created and linked to the Card.",
            "The Tag was created and linked to the Card.",
        ]
    )
...
async def test_mutation_repair_loop_stops_after_five_rounds(e2e_harness):
    ...
    advisor, provider = e2e_harness.advisor([
        route_turn("workspace_mutator"),
        *([invalid_turn] * 6),
        "I could not prepare the requested change after five repair attempts. No unfinished operation was applied.",
    ])
